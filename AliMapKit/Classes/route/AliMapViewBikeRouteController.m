//
//  AliMapViewBikeRouteController.m
//  AliMapKit
//
...
@interface AliMapViewBikeRouteController ()<AMapSearchDelegate>{
...
@implementation AliMapViewBikeRouteController
...
- (void)viewDidLoad {
    ...
    CLLocationCoordinate2D destiCoordinate; //回龙观
    destiCoordinate.longitude = 116.3333154349;
    destiCoordinate.latitude  = 40.0787987409;
    
    AMapRidingRouteSearchRequest *bikeRouteRequest = [[AMapRidingRouteSearchRequest alloc] init];
    bikeRouteRequest.type = 0;
    bikeRouteRequest.origin = [AMapGeoPoint locationWithLatitude:startCoordinate.latitude
                                                       longitude:startCoordinate.longitude];
    bikeRouteRequest.destination = [AMapGeoPoint locationWithLatitude:destiCoordinate.latitude
                                                            longitude:destiCoordinate.longitude];
    
    
    //发起请求,开始POI的骑行出行路线查询检索
    [_POISearchManager AMapRidingRouteSearch:bikeRouteRequest];
...
- (void)onRouteSearchDone:(AMapRouteSearchBaseRequest *)request response:(AMapRouteSearchResponse *)response{
    ...
    [response.route.paths enumerateObjectsUsingBlock:^(AMapPath * _Nonnull pathObj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [self.routeM appendFormat:@"预计耗时：%ld分钟\n",pathObj.duration/60];
        [self.routeM appendFormat:@"此方案费用：%.1lf元\n",pathObj.tolls];
        ...
        [pathObj.steps enumerateObjectsUsingBlock:^(AMapStep * _Nonnull stepObj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [self.routeM appendFormat:@"此路段预计耗时：%@\n",stepObj.duration>=60?[NSString stringWithFormat:@"%ld分钟",stepObj.duration/60]:[NSString stringWithFormat:@"%ld秒",stepObj.duration]];
...
-(void)createAllSubViews{
    ...
    _deastinationLabel.text = @"目的地：回龙观";
